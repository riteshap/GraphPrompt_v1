...
class ModelBuilder:
        ...
        for epoch in range(self.num_epochs):
            ...
            if epoch in [10, 15]:
                torch.save(model.state_dict(),
                           r"./model_checkpoints/amasum_entity_{}_epoch{}.pt".format(self.aim_asp, epoch))

        torch.save(model.state_dict(), r"./model_checkpoints/amasum_entity_{}.pt".format(self.aim_asp))
        print("over")
    ...
    def _evaluating_process(self):
        ...
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model = EntityClassifier()
        model.load_state_dict(torch.load("./model_checkpoints/amasum_entity_{}.pt".format(self.aim_asp)))
        model = model.to(device)
        model.eval()
...
class PairDataset(Dataset):
    ...
    def _build_sample(self):
        ...
        gold_entites = {}
        for i in range(len(self.gold_pairs)):
...
class PairModelBuilder:
    ...
    def _training_process(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            if epoch in [10, 15]:
                torch.save(model.state_dict(),
                           r"./model_checkpoints/amasum_pair_{}_epoch{}.pt".format(self.aim_asp, epoch))

        torch.save(model.state_dict(), r"./model_checkpoints/amasum_pair_{}.pt".format(self.aim_asp))
        print("over")
    ...
    def _evaluating_process(self):
        ...
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model = PairClassifier()
        model.load_state_dict(torch.load("./model_checkpoints/amasum_pair_{}.pt".format(self.aim_asp)))
        model = model.to(device)
        model.eval()
